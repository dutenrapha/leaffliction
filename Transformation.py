...
def generate_output_dir(full_path, output_dir):
    """
    Substitute the root directory of a full file path
    with a specified output directory, excluding the filename.
    ...
    Returns:
    - str: The modified path with the new root directory,
    excluding the filename.
    """
    # Split the full path into components
    path_components = full_path.split(os.sep)

    # Remove the first component (the original root)
    # and the last component (the filename)
...
def transform(origpath, output_path, show_img):
    ...
    cv2.imwrite(os.path.join(savedir, f"{filename}_gaussian_blur.jpg"),
                blur_gaussiano)

    blur_gray = cv2.cvtColor(blur_gaussiano, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(os.path.join(savedir, f"{filename}_gaussian_blur_bw.jpg"),
                blur_gray)

    _, mask = cv2.threshold(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY),
...
if __name__ == "__main__":
    if len(sys.argv) != 2 and len(sys.argv) != 3:
        ...
        print("Usage: python Transformation.py [path_to_dir] [output_path]:"
              " process all files in path_to_dir and saves in"
              "output_path (must be different)")
        sys.exit(1)
